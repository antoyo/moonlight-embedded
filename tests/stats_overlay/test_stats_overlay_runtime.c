...
int main(void) {
  ...
  stats_overlay_snapshot_set_value(&snapshot.queue_delay_avg_ms, true, 8.0);
  ...
  assert(strstr(state.formatted_lines[12], "host 2.50") != NULL);
  assert(strstr(state.formatted_lines[12], "pacing 33.33 ms") != NULL);
  ...
  assert(strstr(state.formatted_lines[13], "Unavailable") != NULL);
  ...
  assert(strcmp(state.formatted_lines[12], "Estimated latency components: host 2.50 + down 4.00 + assembly 5.00 + queue 8.00 + up 4.00 + pacing 33.33 ms") == 0);
  assert(strcmp(state.formatted_lines[13], "Decoder backlog latency: Unavailable") == 0);
